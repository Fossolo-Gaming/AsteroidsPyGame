...
x = 700
y = 300
ship = window.createSprite(shipImage, x ,y)

clock = Clock()

grid = Grid()

tilesData = [ ['A', "images/tilesA/towerDefense_tile124.png"],
              ['B', "images/tilesA/towerDefense_tile093.png"]]

tileset = Tileset(tilesData)
...
grid.setGrid(tiles, 64, 64, pygame.Rect(50, 50, 924, 628))
grid.drawLines = True
window.addGrid(grid)

toolset = Grid()
toolset.setTileset(tileset)
toolsetTiles = ["AB ", "AB ", "AB "]
toolset.setGrid(toolsetTiles, 64, 64, pygame.Rect(50, 690, 924, 70))
toolset.setPosition(50, 690)
toolset.drawLines = True
window.addGrid(toolset)
...
while quitGame == False:        
    ...
    for test in tests:
        grid.move(test)
        if grid.isCollided(ship.rect, ["A", "B"]):
            grid.setPosition(oldPos[0], oldPos[1])
        else:
            break

    mousePos = pygame.mouse.get_pos()
    cell = grid.pick(mousePos)
    if cell!=None:
        grid.setHighlightedCell(cell)
        buttons = pygame.mouse.get_pressed()
        for index in range(0,3):
            if buttons[index]:
                grid.setTileValue(cell, tileValues[index])

    cell = toolset.pick(mousePos)
    if cell!=None:
        toolset.setHighlightedCell(cell)
        buttons = pygame.mouse.get_pressed()
        for index in range(0,3):
            if buttons[index]:
                tileValues[index] = toolset.getTileValue(cell)
