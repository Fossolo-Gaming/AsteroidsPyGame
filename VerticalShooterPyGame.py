...
# Web page: http://www.fossolo.it/ricreativo/fossolo-gaming/
...
class GameData:
    ...
    def __init__(self, backgroundFilename):
        ...
        GameData.screen.blit(GameData.background, (0, GameData.screen_dims[1]))
        pygame.display.flip()
...
class BasicSprite(pygame.sprite.DirtySprite):
    ...
    def __init__(self, imageId, pos):
        ...
        self.rect.center = floatToInt2D(pos)
...
class MovingSprite(BasicSprite):
    ...
    # onBorderMode:
    # 0 : do nothing. The sprite continue to move outside the screen
    # 1 : rebound. The sprite inverts is speed
    # 2 : block. The sprite speed is set to zero
    # 3 : teleport. Move sprite on opposite side
    # If the sprites touches the screen borders it bounds (is reboundOnBorders flag is enabled)
    def __init__(self, imageId, pos, mode):
        BasicSprite.__init__(self, imageId, pos)
        self.image0 = self.image
        self.vel = [0., 0.]
        self.rotAngle = 0.
        self.image0Angle = 0.
        self.onBorderMode = mode
        self.border = [1., 1.]
    ...
    def update(self):
        ...
        for i in [0, 1]:
            ...
            if self.onBorderMode==3:
                if nv >= float(GameData.screen_dims[i]):
                    nv = 0.
                elif nv < -dims[i]:
                    nv = float(GameData.screen_dims[i])
            elif self.onBorderMode>0:
                # if nv >= (float(GameData.screen_dims[i] - dims[i]) - float(dims[i]) * self.border[i]) or nv < 0. * (self.border[i] - 1.) * float(dims[i]) :
                if nv >= float(GameData.screen_dims[i] - dims[i]) or nv < 0. :
                    if self.onBorderMode==1:
                        self.vel[i] = -self.vel[i]
                        nv = self.rect[i] + 2. * self.vel[i] 
                    if self.onBorderMode==2:
                        self.vel[i] = 0.0           
            self.pos[i] = nv
...
class VariableImageSprite(MovingSprite):
    ...
    def __init__(self, imageIds, index, pos, mode):   
        MovingSprite.__init__(self, imageIds[index], pos, mode)
        self.size = index 
...
class TextSprite(pygame.sprite.DirtySprite):
    ...
    def updateText(self, text):
        ...
        self.rect = self.image.get_rect()
        self.rect.center = floatToInt2D(self.pos)
...
class Asteroids(SubGroup):
    ...
    def create(self, size, x, y, angle, speed):
        asteroid = VariableImageSprite(GameData.imageIds['Asteroid'], size-1, [x,y], 3)
        self.add(asteroid)
...
class Shield(VariableImageSprite):
    ...
    def __init__(self, size, playerShip):
        ...
        VariableImageSprite.__init__(self, GameData.imageIds['Shield'], size-1, pos, 0)        
...
class Bullets(SubGroup):
    ...
    def create(self, pos):
        bullet = MovingSprite(GameData.imageIds['Bullet'][0], pos, 0)
        self.add(bullet)
        return bullet
...
class Player:
    ...
    def reset(self):        
        ...
        self.angle = 0.
        self.ship = MovingSprite(GameData.imageIds['Player'][0], pos, 2)
        self.ship.offset = [0.5, 1.]
    ...
    def moveShip(self, dirX, dirY):
        self.ship.vel[0] = self.shipSpeed * dirX
        self.ship.vel[1] = self.shipSpeed * dirY
    ...
    def fireBullet(self, angle):
        ...
        bullet.rect.center = floatToInt2D([self.ship.rect.center[0] - 50. * xy[0], self.ship.rect.center[1] - 50. * xy[1]])
...
class PowerUp(VariableImageSprite):
    ...
    def __init__(self):
        ...
        VariableImageSprite.__init__(self, imageList, self.bonusType, pos, 0)
        self.vel = [0.0, 3.]
...
class PowerUps(SubGroup):
    ...
    # Every 6 seconds a new powerUp is spawned on a random position on the screen
    
    def __init__(self, game):
        SubGroup.__init__(self, game.sprites)
        self.timer = Timer(False, 6.0)
    ...
    def destroyOnBoundary(self):
        for powerUp in self:
            if int(powerUp.pos[1]) >  GameData.screen_dims[1]:
                powerUp.kill()            
...
class Game():
    ...
    def __init__(self):
        ...
        self.screen_height = GameData.screen_dims[1]
        self.backgroundY = - 2 * self.screen_height
        self.backgroundSpeed = 2        


    def scrollBackground(self):
        self.backgroundY += self.backgroundSpeed
        if self.backgroundY > 0:
            self.backgroundY = - 2 * self.screen_height

    def drawBackground(self):
        GameData.screen.blit(GameData.background, (0, self.backgroundY))
    ...
    def main(self):
        while self.player.lives>0 and self.level<len(Game.levelData):
            ...
            while collision == False:
                ...
                self.powerUps.destroyOnBoundary()
...
gameData = GameData("images/scrollbackground.png")
